...
const SalesLeadForm: React.FC = ({ route }) => {
    ...
    useEffect(() => {
        ...
        if (route?.params?.rowData) {
            ...
            const { firmname, groweraddress, growerreference, leadtype, growercell, areakanal, areamarla, sitelocation, latitude, longitude } = route.params.rowData;

            setFormData((prevFormData) => ({
                ...prevFormData,
                firmname: firmname || '',
                groweraddress: groweraddress || '',
                growerreference: growerreference || '',
                leadtype: leadtype || '',
    ...
    const handleSubmit = async () => {
        const { firmname, groweraddress, growerreference, leadtype, growercell, areakanal, areamarla, sitelocation, latitude, longitude } = formData;
        ...
        const url = route?.params?.rowData ? `${authCtx.apiUrl}/saleslead/update/${route?.params?.rowData.wid}/` : `${authCtx.apiUrl}/saleslead/add`;
        ...
        try {
            ...
            if (response.ok) {
                showDialog(result.message || 'Sales lead added successfully.', 'check-circle');
                setFormData({
                    firmname: '',
                    groweraddress: '',
                    growerreference: '',
