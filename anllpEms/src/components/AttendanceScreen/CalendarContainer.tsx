...
const CalendarContainer = () => {
  ...
  const [_attendanceData, setAttendanceData] = useState<AttendanceRecord[]>([]);
  ...
  const showDialog = useCallback((message: string, icon: string) => {
    setDialogState({
  ...
  const getPastDates = useCallback((startDate: string) => {
    const dates: string[] = [];
        ...
        if (res.ok) {
          ...
          pastDates.forEach(date => {
            ...
            marks[date] = {
              customStyles: {
                container: {
                  backgroundColor: dayOfWeek === 0 ? 'gray' : 'red',
                  borderRadius: 8,
                },
                text: {
                  color: 'white',
  ...
  }, [isFocused, useCtx.apiUrl, useCtx.token, getPastDates, showDialog]);
