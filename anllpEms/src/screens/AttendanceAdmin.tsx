...
export default function AttendanceAdmin() {
  ...
  const formatTime = (time: string | null) => {
    ...
    return `${String(dateObj.getDate()).padStart(2, '0')}/${String(
      dateObj.getMonth() + 1,
    ).padStart(2, '0')}/${String(dateObj.getFullYear()).slice(2)} ${String(
      dateObj.getHours(),
  ...
  const tableHead =
    attendanceData.length > 0
      ? Object.keys(getDisplayColumns(attendanceData[0])).map(
        ...
        <>
          <View style={styles.cardsContainer}>
            <View style={styles.card}>
              <Text style={styles.cardValueText}>{100}</Text>
            ...
            <View style={styles.card}>
              <Text style={styles.cardValueText}>{0}</Text>
          ...
          {/* // In your component's return statement, replace the table section with this: */}
          {attendanceData.length > 0 ? (
                  ...
                  <ScrollView>
                    {attendanceData.map((rowData, rowIndex) => (
                      <View key={rowIndex} style={styles.dataRow}>
                        {Object.entries(getDisplayColumns(rowData)).map(
                          ([key, value], cellIndex) => {
                            ...
                            return (
                              <View
                                key={cellIndex}
                                style={[styles.dataCell, {width: 180}]}>
                                <Text
                        ...
                        <View style={[styles.dataCell, {width: 180}]}>
                          <TouchableOpacity
                            disabled={!rowData.checkInTime}
                            style={[
                              styles.button,
                              {
                                backgroundColor: !rowData.checkInTime
                                  ? COLORS.DARK_GRAY
                                  : COLORS.ACCENT_ORANGE,
                              },
                            ]}
                            onPress={() =>
                              handleActionPress(rowData?.attendanceId)
                            }>
...
const styles = StyleSheet.create({
  ...
  text: {padding: 6, width: 100, flexWrap: 'nowrap'},
  dropdown: {
    height: 50,
    borderColor: 'gray',
    borderWidth: 1,
