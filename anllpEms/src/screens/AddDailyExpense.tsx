...
const AddExpenseScreen: React.FC = () => {
  ...
  const handleSubmit = async () => {
    const {expenseDate, expenseCategory, expenseDescription, amount, image} =
      formData;
    ...
    ImagePicker.launchCamera({mediaType: 'photo'}, response => {
      if (response.didCancel) {
        ...
        if (response.assets && response.assets[0]) {
          ...
          if (image.fileSize !== undefined && image.uri) {
            ...
            if (image.fileSize > MAX_SIZE_KB * 1024) {
              ...
              ImageResizer.createResizedImage(image.uri, 800, 800, 'JPEG', 70)
                .then(resizedImage => {
                  console.log(
                    `Resized Image Size: ${resizedImage.size / 1024} KB`,
                  );
                  setFormData(prev => ({
                    ...prev,
                    image: resizedImage,
            ...
            console.error('Invalid image data: missing fileSize or uri');
          }
        }
      }
    });
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.dropdownContainer}>
            ...
            <Dropdown
              ...
              onChange={item =>
                setFormData(prev => ({...prev, expenseCategory: item.value}))
              }
            />
          </View>
          ...
          <View>
            ...
            <TextInput
              ...
              onChangeText={text =>
                setFormData(prev => ({...prev, expenseDescription: text}))
              }
            />
          </View>
